...
int main () 
{
    ...
    while (WindowShouldClose() == false) // Loop do jogo, fica aberto ate o usario fechar a janela
    {
        ...
        if (game.gameOver)
        {        
            DrawTextEx(font, "GAME OVER", {320, 450}, 38, 2, WHITE); // Se o jogo acabou, mostra essa mensagem
        }
        DrawRectangleRounded({320,55,170,60}, 0.3, 6, azulClaro);

        char scoreText[10]; // Faz o converte da pontuação em texto para ser exibido
        sprintf(scoreText, "%d", game.score);
        Vector2 textSize = MeasureTextEx(font, scoreText, 38, 2);

        DrawTextEx(font, scoreText, {320 + (170 - textSize.x) / 2, 65}, 38, 2, WHITE);
        DrawRectangleRounded({320,215,170,180}, 0.3, 6, azulClaro); // Caixa para mostrar a proxima peça
